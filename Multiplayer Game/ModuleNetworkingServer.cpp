...
void ModuleNetworkingServer::onPacketReceived(const InputMemoryStream &packet, const sockaddr_in &fromAddress)
{
	if (state == ServerState::Listening)
	{
		...
		if (message == ClientMessage::Hello)
		{
			...
			if (proxy != nullptr)
			{
				...
				for (uint16 i = 0; i < networkGameObjectsCount; ++i)
				{
					...
					proxy->replicationManager.create(proxy->gameObject->networkId);
				}
		...
		{
			proxy->secondsSinceLastRecv = 0.0f;
		}
...
void ModuleNetworkingServer::onUpdate()
{
	if (state == ServerState::Listening)
	{
		...
		for (ClientProxy &clientProxy : clientProxies)
		{
			...
			{
				// TODO(you): UDP virtual connection lab session
				clientProxy.secondsSinceLastRecv += Time.deltaTime;
				clientProxy.secondsSinceLastPing += Time.deltaTime;


				if (clientProxy.secondsSinceLastRecv > DISCONNECT_TIMEOUT_SECONDS)
				{
				...
				// TODO(you): World state replication lab session
				clientProxy.secondsSinceLastRep += Time.deltaTime;

				if (clientProxy.secondsSinceLastRep > REPLICATION_INTERVAL_SECONDS)
				{
					OutputMemoryStream packet;
					packet << PROTOCOL_ID;
					packet << ServerMessage::Replication;
					
					packet << clientProxy.nextExpectedInputSequenceNumber; //send notifications about the last processed input data

					//Delivery* del = clientProxy.delManager.writeSequenceNumber(packet); //PENDING
					//del->delegate = new DeliveryDelegate();

					clientProxy.replicationManager.write(packet);

					sendPacket(packet, clientProxy.address);
					clientProxy.secondsSinceLastRep = 0.0f;

				}

				//clientProxy.delManager.processTimedOutPackets(); //PENDING
