...
void ModuleNetworkingClient::onPacketReceived(const InputMemoryStream &packet, const sockaddr_in &fromAddress)
{
	...
	else if (state == ClientState::Connected)
	{
		...
		if (message == ServerMessage::Replication)
		{
			...
			if (nextSequenceNumber == inputDataBack)
			{
				inputDataFront = inputDataBack;
			}

			//delManager.processSequenceNumber(packet);  //PENDING
...
void ModuleNetworkingClient::onUpdate()
{
	...
	else if (state == ClientState::Connected)
	{
		...
		secondsSinceLastRecv += Time.deltaTime;
		...
		if (secondsSinceLastInputDelivery > inputDeliveryIntervalSeconds)
		{
			...
			// Clear the queue
			//inputDataFront = inputDataBack;    //only clear when receive server notification 

			sendPacket(packet, serverAddress);
		}

		// TODO(you): Latency management lab session
		if (secondsSinceLastRecv > DISCONNECT_TIMEOUT_SECONDS)
		{
			disconnect();
		}
